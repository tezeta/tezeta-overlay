...
SRC_URI="https://nginx.org/download/${P}.tar.gz
	...
	nginx_modules_http_mpeg_ts? ( ${HTTP_MPEG_TS_MODULE_URI} -> ${HTTP_MPEG_TS_MODULE_P}.tar.gz )
...
src_configure() {
	...
	if use nginx_modules_http_mpeg_ts; then
		http_enabled=1
		myconf+=( --add-module=${HTTP_MPEG_TS_MODULE_WD} )
	fi
